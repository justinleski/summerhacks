...
/** Split lighthouse + form chrome used by real auth on HomePage. */
export function LoginChrome({
  ...
  return (
    <main className={`login-layout login-layout--${variant}`}>
      ...
      <section
        className="login-layout__panel"
        aria-label={`${copy.eyebrow} login`}
      >
        ...
        <div className="login-form">
          ...
          <h1 className="login-form__title">{title ?? copy.title}</h1>
          <p className="login-form__subtitle">{subtitle ?? copy.subtitle}</p>
          {children}
        </div>
      </section>
    </main>
