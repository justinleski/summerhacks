...
6. Each user taps **Confirm connection** → `POST /api/sessions/:id/confirm`. When **both** members have `confirmedAt`, session becomes `active`.
7. Session stays reopenable via `/session/:id` and Recent connections — no bump required.
...
| `expired` | Window elapsed with no match |
