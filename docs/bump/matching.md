...
Constants live in `packages/shared` (`MATCH_TIME_WINDOW_MS`, `BUMP_EXPIRY_MS`, `BUMP_POLL_INTERVAL_MS`, `BUMP_CANDIDATE_WINDOW_MS`). Client peak threshold is `THRESHOLD = 16` in `BumpMode.tsx`.
...
| Candidate window | 45000ms | Post-expiry anonymous pool + proposal TTL (`BUMP_CANDIDATE_WINDOW_MS`) |
| Cardinality | One-to-one | Closest-in-time pending partner wins (auto-match) |
...
Implemented in `memory-store.ts` / `neon-store.ts` `placesMatch` / `placeSqlClause`:
...
No browser geolocation in MVP. NYC must never match Toronto (different city/region/coords/IP).
...
## Candidate fallback (after auto-match expires)

When the ±2s auto-match window misses, the client stays in an `expired` phase and loads **anonymous** candidates (avatar only — **no displayName**):

1. Same coarse place rules as auto-match
2. Partner bump `server_timestamp` within the last **45s** (`BUMP_CANDIDATE_WINDOW_MS`)
3. Not already `matched`; one row per user (latest bump)

User A taps an avatar → `POST /api/bumps/:id/propose` → User B polls `GET /api/bumps/proposals` and sees A's avatar ("Was this you?"). Accept creates/reuses the normal session flow (`pending_confirm` → both confirm → `active`). Decline dismisses. Proposals expire after the same 45s TTL.
...
While status is searching, the client polls `GET /api/bumps/:id` every **~500ms** until matched or expired. Poll also re-runs `tryMatchBump` so the first arriver can pair when the second posts. After expiry, poll candidates + incoming proposals (and bump status for late accept). Do not poll outside Bump Mode.
